...
def get_player_matches(db: Session, player_id: str, opponent_id: Optional[str], start_year: int, end_year: int, limit: int) -> Optional[dict]:
    ...
    if opponent_id is None:
        ...
        result = db.query(models.Match).join(models.Tournament).filter(models.Tournament.startDate.between(start, end)
                                                                       & (((models.Match.winnerId == player_id) & (models.Match.loserId == opponent_id))
                                                                          | ((models.Match.loserId == player_id) & (models.Match.winnerId == opponent_id)))).limit(limit).all()
...
def get_matches_with_opponent(db: Session, player_id: str, opponent_id: str, limit: int) -> Optional[dict]:
    if player_id == "" or opponent_id == "":
        return None

    result = db.query(models.Match).filter((models.Match.winnerId == player_id & models.Match.loserId == opponent_id) | (
        models.Match.loserId == player_id & models.Match.winnerrId == opponent_id)).limit(limit).all()
